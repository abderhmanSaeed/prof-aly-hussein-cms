...
public class IndexModel : PageModel
{
    ...
    public List<StatItem> Items { get; private set; } = new();

    public async Task OnGetAsync() =>
        Items = await _db.StatItem.Include(x => x.Translations).OrderBy(x => x.SortOrder).ThenBy(x => x.Id).ToListAsync();

    public async Task<IActionResult> OnPostDeleteAsync(int id)
    {
        var entity = await _db.StatItem.FindAsync(id);
        if (entity is not null)
        {
            _db.StatItem.Remove(entity);
    ...
    {
        var items = await _db.StatItem.OrderBy(x => x.SortOrder).ThenBy(x => x.Id).ToListAsync();
