...
public class IndexModel : PageModel
{
    ...
    public List<Qualification> Items { get; private set; } = new();

    public async Task OnGetAsync() =>
        Items = await _db.Qualification.Include(x => x.Translations).OrderBy(x => x.SortOrder).ThenBy(x => x.Id).ToListAsync();

    public async Task<IActionResult> OnPostDeleteAsync(int id)
    {
        var entity = await _db.Qualification.FindAsync(id);
        if (entity is not null)
        {
            _db.Qualification.Remove(entity);
    ...
    {
        var items = await _db.Qualification.OrderBy(x => x.SortOrder).ThenBy(x => x.Id).ToListAsync();
