...
public class UpsertModel : PageModel
{
    ...
    public async Task<IActionResult> OnGetAsync(int? id)
    {
        ...
        if (id is int x)
        {
            entity = await _db.Skill.Include(s => s.Translations).FirstOrDefaultAsync(s => s.Id == x);
            if (entity is null)
            {
                return NotFound();
            }
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        if (Input.Id == 0)
        {
            entity = new Skill
            {
                ...
                SortOrder = (await _db.Skill.MaxAsync(s => (int?)s.SortOrder) ?? -1) + 1,
            };
            _db.Skill.Add(entity);
        }
        else
        {
            entity = await _db.Skill.Include(s => s.Translations).FirstAsync(s => s.Id == Input.Id);
        }
        ...
        foreach (var input in Input.Translations)
        {
            ...
            if (tr is null)
            {
                tr = new SkillTranslation { Culture = input.Culture };
